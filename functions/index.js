...
exports.sendDailyReport = onSchedule(
    ...
    async (event) => {
      ...
      try {
        ...
        const now = new Date();
        const osloOffset = 1; // CET = UTC+1 (adjust for DST if needed)
        ...
        // Expected medicines (simplified list)
        const expectedMeds = [
        ...
        expectedMeds.forEach((exp) => {
          ...
          const isGiven = givenMeds.includes(exp.name);
          if (!isGiven) {
            missingHtml += `<tr>
        ...
        // Sondemat total
        const sondeTotal = sonde.reduce((sum, s) => {
        ...
        bowel.forEach((b) => {
          ...
          const user = b.loggedBy || "";
          bowelHtml += `<li>${timeStr} - ${b.bmAmount || "Ukjent mengde"}, ` +
            `${b.bmConsistency || ""}, ${b.bmColor || ""} ` +
            `${user ? "(" + user + ")" : ""}</li>`;
        });
...
<head>
  ...
  <style>
    ...
    .header { background: linear-gradient(135deg, #4CAF50, #2E7D32); color: white; padding: 24px; text-align: center; }
...
<body>
  <div class="container">
    <div class="header">
      <h1>\ud83d\udccb Dosevakt - Daglig rapport</h1>
      <p>${dateStr}</p>
    </div>
    
    <div class="content">
      <div class="section">
        <div class="section-title">\ud83d\udc8a Medisiner gitt i dag</div>
        ${medicines.length > 0 ? `
        <table>
          <thead>
            <tr>
        ...
        ` : "<p style='color: #888;'>Ingen medisiner registrert i dag</p>"}
        
        ${missingHtml ? `
        <div class="warning">
          <strong>\u26a0\ufe0f Ikke registrert:</strong>
      ...
      <div class="section">
        <div class="section-title">\ud83c\udf7c Sondemat</div>
        ${sonde.length > 0 ? `
        <ul>${sondeHtml}</ul>
        <p style="margin-top: 8px;"><strong>Totalt: ${sondeTotal} ml</strong> ${sondeTotal >= 1300 ? "\u2705" : "\u26a0\ufe0f (M\u00e5l: 1300 ml)"}</p>
        ` : "<p style='color: #888;'>Ingen sondemat registrert i dag</p>"}
      </div>
      
      <div class="section">
        <div class="section-title">\ud83d\udca9 Avf\u00f8ring</div>
        ${bowel.length > 0 ? `<ul>${bowelHtml}</ul>` : "<p style='color: #888;'>Ingen registreringer i dag</p>"}
      </div>
      
      <div class="section">
        <div class="section-title">\ud83d\udca7 Vannlating</div>
        <p>${urine.length} registrering${urine.length !== 1 ? "er" : ""} i dag</p>
      </div>
      ...
      <div class="section">
        ...
        <div class="stats">
          <div class="stat-box">
            ...
            <div class="stat-label">Medisiner gitt</div>
          </div>
          <div class="stat-box">
            <div class="stat-value">${sondeTotal} ml</div>
            <div class="stat-label">Sondemat</div>
          </div>
          <div class="stat-box">
            <div class="stat-value">${bowel.length}</div>
            <div class="stat-label">Avf\u00f8ringer</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="footer">
      Generert automatisk av Dosevakt<br>
      <a href="https://jensapp-14069.web.app" style="color: #4CAF50;">\u00c5pne appen</a>
    </div>
  </div>
</body>
</html>
